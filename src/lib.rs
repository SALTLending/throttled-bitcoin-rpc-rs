...
jsonrpc_client!(pub struct BitcoinRpcClient {
    ...
    enum:
        #[cfg(all(not(feature = "ltc"), not(feature = "doge")))] pub fn getblock(&self, header_hash: &str, verbosity: u8) -> Result<Zero(SerializedData)|One(Block)|Two(FullBlock)>;
        #[cfg(any(feature = "ltc", feature = "doge"))] pub fn getblock(&self, header_hash: &str, verbosity: bool) -> Result<False(SerializedData)|True(Block)>;
        #[cfg(all(not(feature = "ltc"), not(feature = "doge")))] pub fn getrawtransaction(&self, txid: &str, verbose: bool) -> Result<False(SerializedData)|True(Transaction)>;
        #[cfg(any(feature = "ltc", feature = "doge"))] pub fn getrawtransaction(&self, txid: &str, verbose: u8) -> Result<Zero(SerializedData)|One(Transaction)>;
});
